...
public static class S1_User_Log
{
    // 로그 파일이 생성될 경로 관리용
    ...
    {
        // 폴더이름
        string t_Directory = "/S1_UserLog";

        // 로그 파일 이름
        string t_FileName = "S1-User-Log-" + DateTime.Now.ToString("dd.MM.yyyy HH.mm.ss") + ".csv";
    ...
    public static void Log(string actor, string behavior, string taskStimuli, string totYellowHandHesitation, string totBlueHandHesitation, string totRedHandHesitation, string totGreenHandHesitation,
                           string response, string interferenceResponse,
                           string cumulativeLeft_ControllorPos, string cumulativeRight_ControllorPos, string actionTime, string incorrect, string correct)
    {
        // 시스템 로그도 기록
        //Debug.Log(_logmsg);
        ...
        // 로그 내용 앞에 시간 추가  
        string t_Logfrm = DateTime.Now.ToString("mm:ss") + ";" + actor + ";" + behavior + ";"+ taskStimuli + ";" + totYellowHandHesitation + ";" +
                          totBlueHandHesitation + ";" + totRedHandHesitation + ";" + totGreenHandHesitation + ";" +
                          response + ";"+ interferenceResponse + ";"+ cumulativeLeft_ControllorPos+ ";"+ cumulativeRight_ControllorPos + ";" +
                          actionTime + ";" + incorrect + ";" + correct;
